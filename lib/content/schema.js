 ...
 * @property {string} [//]  Scaffold note. Not rendered.
 ...
 * @property {"low" | "medium" | "high" | "unknown"} costBand
...
export const countrySchema = z
  .object({
    // JSON has no comment syntax and the schema is strict, so a scaffold note
    // needs a key of its own. "//" is the conventional JSON comment idiom.
    "//": z.string().optional(),
    slug,
    name: text,
    iso2: z
    ...
    // "unknown" is a real state, not a gap to paper over: a country whose cost
    // band has not been researched must not match a cost-band filter, because
    // a placeholder band makes the filter return wrong results.
...
/**
 * A glossary term. Carries the same provenance burden as a program: an
 * immigration glossary makes factual claims just as a program guide does, so
 * sources and lastReviewed are required, not optional.
 *
 ...
 * @property {string} longform  May be empty.
 * @property {string[]} relatedPrograms  Slugs of program records.
 * @property {string[]} relatedTerms     Slugs of other glossary records.
...
/**
 * A guide. Its MDX body is attached separately by the loader, the same way a
 * programme's is.
 *
 ...
 * @property {import("./intents.js").IntentSlug | null} intent
 * @property {string[]} countries  Slugs of country records. May be empty.
 * @property {Author} author
 * @property {string} lastReviewed
 * @property {Source[]} sources
 * @property {number} readingTime  Minutes, a plain estimate.
 * @property {string[]} relatedPrograms  Slugs of program records.
...
export const guideSchema = z
  .object({
    ...
    title: text,
    standfirst: text,
    // A standfirst is written for the top of an article and is the wrong
    // length for a card. cardSummary is the one-line form, set only on the
    // guides that surface as cards; everything else falls back to standfirst.
    cardSummary: text.optional(),
    intent: z.enum(INTENT_SLUGS).nullable(),
    countries: z.array(slug),
    author,
    lastReviewed: isoDate,
    sources,
...
/**
 * A news update. Surfaces on the country and programme pages it names, which
 * is the entire reason `countries` and `programs` are relationships rather
 * than free text.
 *
 * @typedef {Object} NewsUpdate
 * @property {string} slug
 * @property {string} title
 * @property {string} body  Plain prose, not MDX — a news item is short enough
 *   not to need it.
 * @property {string[]} countries  Slugs of country records.
 * @property {string[]} programs   Slugs of program records.
 * @property {string} effectiveDate  ISO date the change takes or took effect.
 * @property {Source[]} sources
 * @property {Author} author
 * @property {string} lastReviewed
 */
