...
import { countrySchema, parseOrThrow, programSchema } from "./schema.js";
...
async function loadCountry(slug) {
  ...
  if (cache.has(key)) {
    return /** @type {import("./schema.js").Country} */ (cache.get(key));
  }
  ...
  const record = deepFreeze(country);
...
async function loadProgram(country, intent, slug) {
  ...
  if (cache.has(key)) {
    return /** @type {import("./schema.js").Program} */ (cache.get(key));
  }
  ...
  const record = deepFreeze({ ...frontmatter, body: parsed.content });
  cache.set(key, record);
  return record;
}
...
async function programIndex() {
  ...
  if (cache.has(key)) {
    return /** @type {Array<{ country: string, intent: string, slug: string }>} */ (
      cache.get(key)
    );
  }
  ...
  const index = deepFreeze(entries);
  cache.set(key, index);
  return index;
}
